...
class Bayes_Classifier:
    ...
    def __init__(self, trainDirectory = "movies_reviews/"):
        ...
        self.trainingFileCount=len(IfileList)
        self.priorPositive=countPositive/(countPositive+countNegative)
        self.priorNegative=countNegative/(countPositive+countNegative)


        if os.path.exists("positive_reviews_best") and os.path.exists("negative_reviews_best") :
            self.dictPositive=self.load("positive_reviews_best")
            self.dictNegative=self.load("negative_reviews_best")
        
        else:
            self.dictPositive={}
            self.dictNegative={}
            self.save(self.dictPositive,"positive_reviews_best")
            self.save(self.dictNegative,"negative_reviews_best")
        # self.dictPositive={}
        # self.dictNegative={}
    ...
    def train(self):   
       ...
       #counts for unigrams and bigrams calculation 
        for word in self.dictPositive.keys():
            if " " not in word:
                featureFrequencyPositive+=self.dictPositive[word]
            else:
                bigramPos+=self.dictPositive[word]

        for word in self.dictNegative.keys():
            if " " not in word:
                featureFrequencyNegative+=self.dictNegative[word]
            else:
                bigramNeg+=self.dictNegative[word]
        
   
        
           # applying add one Laplace's Smoothening for Unigrams and Bigrams
        for word in self.dictPositive.keys():
            if " " not in word:
                if word in self.dictNegative.keys():
                ...
                # words=word.split()
                if word in self.dictNegative.keys():
                    self.dictPositive[word]=((self.dictPositive[word]/(bigramPos)))
                else:
                    self.dictPositive[word]=(((self.dictPositive[word]+1)/(bigramPos+1)))
                    self.dictNegative[word]=((1/(bigramNeg+1)))
                    bigramPos+=1
                    bigramNeg+=1
        for word in self.dictNegative.keys():
            if " " not in word:
                ...
                # words=word.split()
                if word in self.dictPositive.keys():
                    self.dictNegative[word]=((self.dictNegative[word]/(bigramNeg)))
                else:
                    self.dictNegative[word]=(((self.dictNegative[word]+1)/(bigramNeg+1)))
                    self.dictPositive[word]=((1/(bigramPos+1)))
                    bigramPos+=1
                    bigramNeg+=1
        ...
        self.dictPositive["*length*"]=posLength
        self.dictNegative["*length*"]=negLength       
    ...
    def classify(self, sText):
        ...
        tText=self.tokenize(sText)
        #Underflow avoided using Log
        for word in tText:
         
            if word =="*length*":
                condProbabilityPositive+=math.log(len(sText)/self.dictPositive[word])
                condProbabilityNegative+=math.log(len(sText)/self.dictNegative[word]) 
            else:
                if word in self.dictPositive.keys():
    ...
    def tokenize(self, sText): 
       ...
       #Adding bigrams to Ltokens
        for i in range(len(lTokens)-1, 0, -1):
            lTokens.append(lTokens[i-1]+" "+lTokens[i])
